...
function ShopSpriteCursor:create(x, y, z, north, sprite)
    ...
    shop:transmitCompleteItemToServer()  -- Sync to all clients
end
```
...
- ✅ `transmitCompleteItemToServer()` syncs state to all clients
...
shop:transmitCompleteItemToServer()  -- Synced to server save
```
