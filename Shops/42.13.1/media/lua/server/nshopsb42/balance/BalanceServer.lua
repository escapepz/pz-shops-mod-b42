...
function BServer.Transfer(player, args)
	...
	if account.coin < coin then
		SharedLogger.log(
			"Shops",
			string.format("[SERVER] Transfer REJECTED: insufficient coin (%d < %d)", account.coin, coin)
		)
		return
	end
	...
	if now - rr.lastTs < 500 then
		SharedLogger.log(
			"Shops",
			string.format("[SERVER] Transfer REJECTED: recipient throttle %d < 500", now - rr.lastTs)
		)
		return
	end
	rr.lastTs = now
