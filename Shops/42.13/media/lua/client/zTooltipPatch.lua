...
local function injectTooltip(self)
    ...
    if price then
        ...
        self:drawRectBorder(0, 0, self.width, self.height + 10, self.borderColor.a, self.borderColor.r,
            self.borderColor.g, self.borderColor.b);
        if isSpecialCoin then coinImg = Currency.CoinsTexture.SpecialCoin end
        self.tooltip:DrawTextureScaledAspect(coinImg.texture, x - 10, y + fontConfig[fontSize].iconY, coinImg.scale,
            coinImg.scale, 1, 1, 1, 1)
    ...
    self.tooltip:DrawTextureScaledAspect(wallet.texture, x - 10, y + fontConfig[fontSize].iconY, wallet.scale,
        wallet.scale, 1, 1, 1, 1)
...
function ISToolTipInv:render()
    if not ISContextMenu.instance or not ISContextMenu.instance.visibleCheck then
        injectTooltip(self)
        -- Skip vanilla render for shop preview items (no character context)
        if self.item then
            local modData = self.item:getModData()
            if modData.belongsTo or modData.price then
                return
            end
        end
        pcall(function() oldRender(self) end)
    end
end
