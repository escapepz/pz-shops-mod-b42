...
function ContainerViewerUI:filter()
	...
	local foundCount = SHOPSB42.Currency.format(#self.containertems.items)
...
function ContainerViewerUI:onMouseMoveContainerItem(dx, dy)
	...
	if list:isMouseOver() then
		...
		if not rowIndex then
			self.parent:toggleTooltip(false)
			return
		end
...
function ContainerViewerUI:createChildren()
	...
	self.filterEntry =
		ISTextEntryBox:new("", getTextManager():MeasureStringX(UIFont.Small, UIText.Search) + 30, 40, 120, 1)
...
function ContainerViewerUI:new(x, y, _width, _height, container)
	local o = {}
	if x == 0 and y == 0 then
