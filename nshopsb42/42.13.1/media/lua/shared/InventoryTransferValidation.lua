...
function InventoryTransferValidation.validateShopOwnership(character, srcContainer, destContainer)
    ...
    local function validateContainerOwnership(container, containerName)
        if not container then
            writeLog("Shops", "[InventoryTransferValidation] " .. containerName .. " is nil")
            return true
        end

        local containerType = tostring(container:getClass())
        writeLog("Shops", "[InventoryTransferValidation] " .. containerName .. " type=" .. containerType)

        local parent = container:getParent()
        if not parent then
            writeLog("Shops", "[InventoryTransferValidation] " .. containerName .. " has no parent")
            return true
        end

        local parentType = tostring(parent:getClass())
        writeLog("Shops", "[InventoryTransferValidation] " .. containerName .. " parent type=" .. parentType)
        ...
        writeLog("Shops", "[InventoryTransferValidation] " .. containerName .. " parentModData={" .. modDataStr .. "}")

        if parentModData and parentModData.owner then
            writeLog("Shops",
                "[InventoryTransferValidation] " .. containerName .. " found owner - owner=" ..
                parentModData.owner .. ", username=" .. username)
            ...
            if character:isAccessLevel("Admin") then
                writeLog("Shops", "[InventoryTransferValidation] Admin override for " .. containerName)
                isOwner = true
            end
    ...
    if not srcValid then
        writeLog("Shops", "[InventoryTransferValidation] Transfer rejected - player does not own source container")
        return false
    end
    ...
    if not destValid then
        writeLog("Shops", "[InventoryTransferValidation] Transfer rejected - player does not own destination container")
        return false
    end

    writeLog("Shops", "[InventoryTransferValidation] Transfer allowed")
    return true
end
