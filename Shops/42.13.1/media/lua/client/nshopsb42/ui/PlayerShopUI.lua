...
local PlayerShopTabUI = SHOPSB42.PlayerShopTabUI
...
function PlayerShopUI:onMouseMove(dx, dy)
	...
	if PlayerShopUI.instance.cartItems:isMouseOver() then
		return
	end
...
function PlayerShopUI:createChildren()
	...
	self.clearCartButton =
		ISButton:new((self.width / 2) + 380, y + 280, 80, 25, UIText.ClearCart, self, PlayerShopUI.clearCartBtn)
	self.clearCartButton:initialise()
	self:addChild(self.clearCartButton)

	self.buyCartButton =
		ISButton:new((self.width / 2) + 200, y + 350, 80, 25, UIText.BuyCart, self, PlayerShopUI.buyCartBtn)
	...
	self.cancelBuyButton =
		ISButton:new((self.width / 2) + 200, y + 350, 80, 25, UIText.Cancel, self, PlayerShopUI.cancelBuyBtn)
	...
	self.balanceLabel =
		ISLabel:new(x + 490, 20, PlayerShopUI.SMALL_FONT_HGT, UIText.Balance, 1, 1, 1, 1, UIFont.Medium, true)
	...
	self.totalLabel =
		ISLabel:new(x + 490, y + 280, PlayerShopUI.SMALL_FONT_HGT, UIText.Total, 1, 1, 1, 1, UIFont.Medium, true)
	self:addChild(self.totalLabel)

	self.totalCoinLabel =
		ISLabel:new(x + 560, y + 280, PlayerShopUI.SMALL_FONT_HGT, "0", 1, 1, 1, 1, UIFont.Medium, true)
	...
	self.balanceSpecialCoinLabel =
		ISLabel:new(x + 575, 45, PlayerShopUI.SMALL_FONT_HGT, "0", 1, 1, 1, 1, UIFont.Medium, true)
...
function PlayerShopUI:new(x, y, _width, _height, player, shopOwner)
	local o = {}
	if x == 0 and y == 0 then
		x = (getCore():getScreenWidth() / 2) - (_width / 2)
		y = (getCore():getScreenHeight() / 2) - (_height / 2)
	end
	---@class ISCollapsableWindow
	o = ISCollapsableWindow:new(x, y, _width, _height)
	setmetatable(o, self)
